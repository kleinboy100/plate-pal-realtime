...
-- Add staff RLS policy for meal_recipes
CREATE POLICY "Staff can manage meal recipes"
...
-- Create function to auto-deduct ingredients from ingredient_stock when order is confirmed
...
DROP TRIGGER IF EXISTS trigger_deduct_ingredients ON public.orders;
CREATE TRIGGER trigger_deduct_ingredients
  AFTER UPDATE ON public.orders
  FOR EACH ROW
  EXECUTE FUNCTION public.deduct_ingredients_on_order();
