ALTER TABLE public.orders
  ...
  ADD COLUMN IF NOT EXISTS delivery_location_accuracy_m numeric,
  ADD COLUMN IF NOT EXISTS delivery_place_id text,
  ADD COLUMN IF NOT EXISTS delivery_address_source text;

ALTER TABLE public.orders
  DROP CONSTRAINT IF EXISTS orders_delivery_latitude_range_check,
  DROP CONSTRAINT IF EXISTS orders_delivery_longitude_range_check,
  DROP CONSTRAINT IF EXISTS orders_delivery_location_accuracy_check,
...
ALTER TABLE public.orders
  ...
  ADD CONSTRAINT orders_delivery_location_accuracy_check
    CHECK (delivery_location_accuracy_m IS NULL OR (delivery_location_accuracy_m >= 0 AND delivery_location_accuracy_m <= 50000)),
  ADD CONSTRAINT orders_delivery_address_source_check
    CHECK (delivery_address_source IS NULL OR delivery_address_source IN ('search', 'current_location', 'pin', 'manual'));
...
DROP FUNCTION IF EXISTS public.create_validated_order(uuid, text, text, text, jsonb, text, numeric, numeric);
DROP FUNCTION IF EXISTS public.create_validated_order(uuid, text, text, text, jsonb, text, numeric);
DROP FUNCTION IF EXISTS public.create_validated_order(uuid, text, text, text, jsonb, text);
DROP FUNCTION IF EXISTS public.create_validated_order(uuid, text, text, text, jsonb);
...
CREATE OR REPLACE FUNCTION public.create_validated_order(
  ...
  p_delivery_address_source text DEFAULT 'manual'
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
