...
DECLARE
  ...
  v_promo_end timestamptz := '2026-06-18T23:59:59+02:00';
  v_promo_discount numeric := 0.10;
  v_promo_ids uuid[] := ARRAY[
    '23a48a68-14b2-47db-9b35-42d18ff4202e'::uuid,
    '0fcd845a-0736-4dca-adfb-84ac24f7de37'::uuid
  ];
  v_promo_active boolean := (now() <= v_promo_end);
  v_item_id uuid;
BEGIN
  v_user_id := auth.uid();
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    IF v_promo_active AND v_item_id = ANY(v_promo_ids) THEN
      v_unit_price := round(v_menu_item.price * (1 - v_promo_discount), 2);
    END IF;
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    VALUES (v_order_id, v_item_id, (v_item->>'quantity')::integer, v_unit_price, v_menu_item.name);
  END LOOP;

  -- Auto-accept the order: move straight to confirmed so the kitchen does not
  -- have to press Accept. Done as an UPDATE so existing triggers (stock
  -- deduction, delivery phone request message) still fire.
